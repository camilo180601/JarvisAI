...
@tool(
    ...
    description='Reglas por hábitos del SO: add_rule, list_rules, delete_rule, trigger_check. Triggers: cpu_high, ram_high, time_of_day, app_open.',
    parameters={'type': 'OBJECT',
     'properties': {'action': {'type': 'STRING',
                               'description': 'add_rule (añadir regla) | list_rules (listar) | '
                                              'delete_rule (eliminar) | trigger_check (evaluar reglas '
                                              'activas)'},
                    'rule_name': {'type': 'STRING',
                                  'description': 'Nombre identificativo de la regla de automatización'},
                    'trigger': {'type': 'STRING',
                                'description': 'Disparador: cpu_high | ram_high | time_of_day | '
                                               'app_open'},
                    'trigger_value': {'type': 'STRING',
                                      'description': "Valor del disparador (ej. '85' para 85% cpu, "
                                                     "'22:00' para hora, 'chrome.exe' para app)"},
                    'action_to_take': {'type': 'STRING',
                                       'description': "Acción a ejecutar (ej. 'optimize_ram', "
                                                      "'mute_system', 'run_script')"}},
     'required': ['action']},
)
